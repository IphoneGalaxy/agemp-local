            ...
            const ClientView = ({ clientData, availableMoney, state, actions, utils }) => {
                ...
                const [paymentKind, setPaymentKind] = useState(FinanceEngine.CLIENT_PAYMENT_KIND.AUTOMATIC);
                ...
                const [showEditPublicKeyModal, setShowEditPublicKeyModal] = useState(false);
                const [clientPublicKeyInput, setClientPublicKeyInput] = useState(clientData.publicKey || '');

                const handleSaveClientPublicKey = () => {
                    const cleanKey = clientPublicKeyInput.trim().toUpperCase();
                    if (cleanKey && !cleanKey.startsWith('PUB-')) {
                        showToast('⚠️ A chave pública deve começar com "PUB-".');
                        return;
                    }
                    const updated = clients.map(c => c.id === clientData.id ? { ...c, publicKey: cleanKey || undefined } : c);
                    setClients(updated);
                    setShowEditPublicKeyModal(false);
                ...
                const openPaymentForm = (loan) => {
                    ...
                    setPaymentKind(FinanceEngine.CLIENT_PAYMENT_KIND.AUTOMATIC);
                    setPaymentAmount('');
                };
                ...
                const handleAddPayment = (e) => {
                    ...
                    const updatedClients = clients.map(c => {
                        if (c.id === clientData.id) {
                            return {
                                ...c,
                                loans: c.loans.map(l => l.id === payingLoanId ? { ...l, payments: [...l.payments, {
                                    id: generateId(), date: paymentDate, amount: Number(paymentAmount), kind: paymentKind
                                }] } : l)
                            };
                        }
                        return c;
                        ...
                        if (c.id === clientData.id) {
                            return {
                                ...
                                loans: c.loans.map(l => l.id === editingPayment.loanId ? {
                                    ...l,
                                    payments: l.payments.map(p => p.id === editingPayment.id ? {
                                        ...p, date: editingPayment.date, amount: Number(editingPayment.amount), kind: editingPayment.kind
                                    } : p)
                                } : l)
                            };
                        }
                        ...
                        if (loan.isPaidOff) {
                            ...
                            if (loan.processedPayments.length > 0) {
                                loan.processedPayments.forEach(p => {
                                    text += `   🟢 ${formatDate(p.date)}: ${paymentKindLabel(p.kind)} de ${formatMoney(p.amount)}\n`;
                ...
                return (
                    <div className="p-4 flex flex-col h-screen bg-gray-50 absolute top-0 left-0 w-full z-10 overflow-y-auto pb-24">
                        ...
                        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-6 text-center">
                            ...
                            {/* Identidade e Chave Pública do Cliente */}
                            <div className="mt-3.5 pt-3 border-t border-gray-100 flex items-center justify-between gap-2 text-left">
                                <div className="flex items-center gap-1.5 overflow-hidden">
                                    <span className="text-base">🪪</span>
                                    {clientData.publicKey ? (
                                        <div className="overflow-hidden">
                                            <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Chave Pública Vinculada</p>
                                            <p className="text-xs font-mono font-bold text-blue-700 truncate select-all">
                                                {clientData.publicKey}
                                            </p>
                                        </div>
                                    ) : (
                                        <div>
                                            <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Chave Pública</p>
                                            <p className="text-xs text-gray-400 italic">Nenhuma chave vinculada</p>
                                        </div>
                                    )}
                                </div>
                                <button
                                    type="button"
                                    onClick={() => {
                                        setClientPublicKeyInput(clientData.publicKey || '');
                                        setShowEditPublicKeyModal(true);
                                    }}
                                    className="text-xs font-bold text-blue-700 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 border border-blue-200 px-3 py-1.5 rounded-xl transition-colors shrink-0"
                                >
                                    {clientData.publicKey ? 'Alterar Chave' : '+ Vincular Chave'}
                                </button>
                            </div>

                            <div className="mt-4 grid grid-cols-3 gap-2">
                                <button data-testid="cliente-btn-emprestimo" onClick={() => setShowNewLoanForm(!showNewLoanForm)} className="bg-blue-600 hover:bg-blue-700 text-white px-2 py-2.5 rounded-xl text-xs font-bold shadow-sm flex items-center justify-center gap-1 transition-colors">
                                    <span>➕</span>
                                    <span>Empréstimo</span>
                                </button>
                                <button onClick={generateStatement} className="bg-gray-800 hover:bg-gray-900 text-white px-2 py-2.5 rounded-xl text-xs font-bold shadow-sm flex items-center justify-center gap-1 transition-colors">
                                    <span>📋</span>
                                    <span>Extrato</span>
                                </button>
                                <button 
                                    type="button"
                                    onClick={() => openClientBackupModal && openClientBackupModal(clientData)} 
                                    className="bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white px-2 py-2.5 rounded-xl text-xs font-bold shadow-sm flex items-center justify-center gap-1 transition-colors"
                                    title="Gerar backup dos empréstimos para o cliente"
                                >
                                    <span>📦</span>
                                    <span>Backup</span>
                                </button>
                                        ...
                                        {loan.isPaidOff ? (
                                            ...
                                            <>
                                                <div className="flex justify-between items-end mb-3">
                                                    ...
                                                    <button onClick={() => openPaymentForm(loan)} className="bg-green-100 text-green-700 px-4 py-2 rounded-xl text-sm font-bold shadow-sm border border-green-200">+ Registrar recebimento</button>
                                                </div>
                                                            ...
                                                            {editingPayment && editingPayment.id === p.id ? (
                                                                ...
                                                                <div className="flex justify-between items-center">
                                                                    <div>
                                                                        ...
                                                                        <p className="text-[10px] text-gray-500 ml-3 mt-0.5">{paymentKindLabel(p.kind)} · Juros: {formatMoney(p.interestPaid)} | Abateu: {formatMoney(p.amortized)}</p>
                                                                    </div>
                                                                    <div className="flex gap-2 text-gray-400">
                                                                        <button onClick={() => setEditingPayment({loanId: loan.id, id: p.id, date: p.date, amount: p.amount, kind: p.kind || FinanceEngine.CLIENT_PAYMENT_KIND.AUTOMATIC})} className="hover:text-blue-600 p-1"><IconEdit /></button>
                            ...
                            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                                <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-slate-100 animate-scale-in">
                                    ...
                                    <p className="text-xs text-slate-500 mb-4 leading-relaxed">
                                        Vincule a Chave Pública de <strong>{clientData.name}</strong> para que ele possa abrir com exclusividade os backups que você enviar para ele.
                                    </p>
                                    ...
                                    <div className="space-y-3">
                                        <div>
                                            ...
                                            <input 
                                                type="text" 
                                                value={clientPublicKeyInput}
                                                onChange={(e) => setClientPublicKeyInput(e.target.value.toUpperCase())}
                                        ...
                                        <div className="flex gap-2 pt-2">
                                            <button
                                                type="button"
                                                onClick={() => setShowEditPublicKeyModal(false)}
                                            ...
                                            <button
                                                type="button"
                                                onClick={handleSaveClientPublicKey}
